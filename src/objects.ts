...
export function isCorrect(question: Question, answer: string): boolean {
    if (
        answer.toLowerCase().trim() === question.expected.trim().toLowerCase()
    ) {
        return true;
    } else {
        return false;
...
export function toMarkdown(question: Question): string {
    let first_two_lines = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        first_two_lines = first_two_lines + "\n" + "- ";
        const the_options = question.options.join("\n" + "- ");
        return first_two_lines + the_options;
    }
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newQuestion = { ...oldQuestion, id: id };
